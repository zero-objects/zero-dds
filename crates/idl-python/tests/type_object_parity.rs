...
//!
//! Proves three things about `idlc python`'s new `<Class>.TYPE_OBJECT` byte
//! constant:
...
//!     `idl-rust` emits for the SAME IDL — i.e. a Python writer and a Rust
//!     writer advertise the IDENTICAL `TypeIdentifier` (the whole point of #24).
...
/// Extracts the `0xNN` bytes of the `.TYPE_OBJECT = bytes([ ... ])` literal in a
/// generated Python module.
fn extract_py_type_object(py: &str) -> Vec<u8> {
    let start = py
        .find(".TYPE_OBJECT = bytes([")
        .expect("no .TYPE_OBJECT literal in Python output");
    let rest = &py[start..];
    let close = rest.find("])").expect("unterminated TYPE_OBJECT literal");
...
fn assert_parity(src: &str, struct_name: &str) {
    ...
    // 1. Python-emitted bytes == shared golden.
    let py = generate_python_module(&spec, &PythonGenOptions::default()).expect("python gen");
    let py_bytes = extract_py_type_object(&py);
    let golden = golden_bytes(src, struct_name);
    assert_eq!(
        py_bytes, golden,
        "Python TYPE_OBJECT bytes != shared golden for {}",
        struct_name
    );

    // 3. COMPLETE TypeObject discriminator.
    assert_eq!(py_bytes[0], 0xF2, "emitted TypeObject is not EK_COMPLETE");

    // 2. MD5-14(python bytes) == idl-rust's advertised TYPE_IDENTIFIER hash.
    let rust = generate_rust_module(&spec, &RustGenOptions::default()).expect("rust gen");
    let rust_hash = extract_rust_identifier_hash(&rust);
    let md5 = zerodds_foundation::md5(&py_bytes);
    assert_eq!(
        &md5[..14],
        rust_hash.as_slice(),
        "MD5-14 of Python TYPE_OBJECT bytes != idl-rust TYPE_IDENTIFIER hash for {} (cross-binding identifier mismatch)",
        struct_name
    );
}
