...
//!
//! Proves three things about `idlc csharp`'s new
//! `<Name>TypeSupport.TypeObject` byte constant (`__ZeroDdsTypeObject`):
...
//!     `idl-rust` emits for the SAME IDL — i.e. a C# writer and a Rust writer
...
/// Extracts the `0xNN` bytes of the `__ZeroDdsTypeObject = new byte[] { ... }`
/// array of the first `TypeObject` body in a generated C# source.
fn extract_cs_type_object(cs: &str) -> Vec<u8> {
    let start = cs
        .find("__ZeroDdsTypeObject = new byte[] {")
        .expect("no __ZeroDdsTypeObject array in C# output");
    let rest = &cs[start..];
    let close = rest
        .find("};")
        .expect("unterminated __ZeroDdsTypeObject array");
...
fn assert_parity(src: &str, struct_name: &str) {
    ...
    // 1. C#-emitted bytes == shared golden.
    let cs = generate_csharp(&spec, &CsGenOptions::default()).expect("cs gen");
    let cs_bytes = extract_cs_type_object(&cs);
    let golden = golden_bytes(src, struct_name);
    assert_eq!(
        cs_bytes, golden,
        "C# TypeObject bytes != shared golden for {}",
        struct_name
    );

    // 3. COMPLETE TypeObject discriminator.
    assert_eq!(cs_bytes[0], 0xF2, "emitted TypeObject is not EK_COMPLETE");

    // 2. MD5-14(cs bytes) == idl-rust's advertised TYPE_IDENTIFIER hash.
    let rust = generate_rust_module(&spec, &RustGenOptions::default()).expect("rust gen");
    let rust_hash = extract_rust_identifier_hash(&rust);
    let md5 = zerodds_foundation::md5(&cs_bytes);
    assert_eq!(
        &md5[..14],
        rust_hash.as_slice(),
        "MD5-14 of C# TypeObject bytes != idl-rust TYPE_IDENTIFIER hash for {} (cross-binding identifier mismatch)",
        struct_name
    );
}
