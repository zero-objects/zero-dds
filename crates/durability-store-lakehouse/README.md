# `zerodds-durability-store-lakehouse`

DuckDB lakehouse cold adapter for the ZeroDDS Durability-Service: in-process SQL (warehouse) + Parquet export (lake). Implements zerodds-durability-store::DurabilityStore.
...
```toml
[dependencies]
zerodds-durability-store-lakehouse = { path = "../path/to/durability-store-lakehouse" }
# or, when published:
# zerodds-durability-store-lakehouse = "0.x"
```

## Tests

```bash
cargo test -p zerodds-durability-store-lakehouse
```
