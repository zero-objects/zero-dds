...
fn apply_cli_overrides(cfg: &mut DaemonConfig, args: CliArgs) {
    ...
    if !args.topics.is_empty() {
        for name in args.topics {
            // Default: bidir, ws_path via default_ws_path, type_name == name.
            let topic = zerodds_websocket_bridge::daemon::config::TopicConfig {
                name: name.clone(),
                type_name: name.clone(),
                direction: "bidir".to_string(),
                ws_path:
                    zerodds_websocket_bridge::daemon::config::default_ws_path(&name),
                ..Default::default()
            };
...
mod tests {
    ...
    fn empty_args_leaves_config_unchanged() {
        ...
        apply_cli_overrides(&mut c, CliArgs::default());
