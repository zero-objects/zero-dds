...
pub unsafe extern "C" fn zerodds_builtin_subscriber_destroy(bs: *mut ZeroDdsBuiltinSubscriber) {
    if !bs.is_null() {
        // SAFETY: Caller-Kontrakt aus get_builtin_subscriber.
...
pub unsafe extern "C" fn zerodds_dp_get_qos(
    ...
    let pp = unsafe { &*p };
    let qos = pp.dp.qos();
    // SAFETY: NULL-Check; nutzt qos_ffi::dp_qos_to_c (existierende API).
...
pub unsafe extern "C" fn zerodds_dw_register_instance(
    ...
    // SAFETY: NULL-Check + key_len==16 oben.
    let raw = unsafe { slice::from_raw_parts(key, 16) };
    // Handle = u64 von ersten 8 Key-Hash-Bytes (deterministisch).
    let mut h = [0u8; 8];
...
pub unsafe extern "C" fn zerodds_dw_get_matched_subscriptions(
    ...
    if !out_handles.is_null() && n > 0 {
        // SAFETY: Caller-Kontrakt: out_handles[0..cap] writeable.
...
pub unsafe extern "C" fn zerodds_dw_commit_loan(
    ...
    // SAFETY: FFI-boundary; pointer validity is the caller's contract per crate-level docs.
    let dwr = unsafe { &*dw };
    // SAFETY: Caller-Kontrakt: ptr aus loan_message, len ist die
    // gleiche len wie aus loan.
    // SAFETY: FFI-boundary; pointer validity is the caller's contract per crate-level docs.
...
mod tests {
    ...
    fn dp_set_get_qos_userdata_roundtrip() {
        ...
        // Read back into properly-sized buffer.
    ...
    fn pub_copy_from_topic_qos_propagates_policies() {
        ...
        // SAFETY: FFI-boundary; pointer validity is the caller's contract per crate-level docs.
        let mut dwqos: ZeroDdsDataWriterQos = unsafe { core::mem::zeroed() };
        assert_eq!(
            // SAFETY: FFI-boundary; pointer validity is the caller's contract per crate-level docs.
