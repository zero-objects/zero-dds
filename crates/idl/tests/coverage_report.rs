...
//!
//! Walks the CST of each fixture, collects actively hit production IDs,
//! compares against all 108 productions in IDL_42 and writes a
//! Markdown report to `tests/coverage_report.md`.
//!
//! Threshold assertion (phase 0): at least 70% of the base productions
//! must be hit by the fixture suite. Aspirational
//! phase-1 goal: 90%.
...
/// Grammar coverage report generator. `#[ignore]` because the threshold
/// depends on spec/fixture growth — run explicitly via
/// `cargo test -p zerodds-idl --test coverage_report -- --ignored`. The
/// generated `coverage_report.md` is committed to the repo, so that the
/// status is visible even without a CI run.
#[test]
#[ignore = "manual run — grammar coverage report; otherwise CI fails non-deterministically as the spec grows"]
fn generate_grammar_coverage_report() {
    ...
    md.push_str(
        "Auto-generated by `cargo test -p zerodds-idl --test coverage_report`. \
         Walks each fixture CST, collects the active production IDs.\n\n",
    );
    md.push_str(&format!(
    ...
    md.push_str("| Fixture | Productions hit | %  |\n");
    md.push_str("|---|---:|---:|\n");
    ...
    if uncovered.is_empty() {
        md.push_str("_None_ — all productions hit.\n");
    } else {
    ...
    assert!(
        ...
        "grammar coverage {pct}% below threshold {COVERAGE_THRESHOLD_PERCENT}%; \
         see {} for untouched productions",
        out_path.display()
    );
}
...
fn collect_covered(src: &str) -> Result<BTreeSet<ProductionId>, String> {
    ...
    for node in walk::preorder(&cst) {
        if let Some(id) = node.production() {
            // Restrict to base-grammar IDs (we skip synth IDs from the
            // compile pass; we want to measure coverage of the
            // OMG-IDL-4.2 grammar).
