...
//!
//! Walkt einen [`CstNode`] (Production-IDs aus [`crate::grammar::idl42`])
//! und produziert eine [`Specification`]. Jede Production hat eine
//! korrespondierende `build_<production>`-Funktion. Listen-Productions
//! (cons-rekursiv mit optionaler Empty-Alternative) werden via
//! [`flatten_list`] in Vec konvertiert.
//!
//! # Fehlermodell
//! Builder-Fehler indizieren *Bugs* — CST kommt aus erfolgreicher
//! Recognition, also sollte die Struktur immer der Grammar entsprechen.
//! Trotzdem [`BuilderError`] statt `panic!`, damit Defekte auffaellbar
//! statt prozessabbrechend sind.
...
/// Builder-spezifischer Fehler. Trifft nur bei CST/Grammar-Mismatch ein.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BuilderError {
    /// Kurze Beschreibung des Mismatchs.
    pub message: String,
    /// Span im Source, wo der CST-Node beginnt.
    pub span: Span,
}
...
impl core::fmt::Display for BuilderError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "AST-Builder-Fehler @ {}: {}", self.span, self.message)
    }
}
...
/// Maximale Verschachtelungs-Tiefe fuer Module + Template-Module-Body.
///
/// Spec OMG IDL 4.2 stellt keine harte Obergrenze, aber alle praktisch
/// auftretenden IDL-Bestaende bleiben weit darunter. Cap schuetzt vor
/// Stack-Overflow bei adversarial Inputs (TS-1-Finding 1,
/// `docs/test-harness/plan.md`).
///
/// 256 ist deutlich oberhalb realistischer Use-Cases (typische
/// Modul-Hierarchien gehen 4-6 tief) und unterhalb der Default-
/// Test-Thread-Stack-Grenze (2 MB / ~1 KB Frame = ~2000 Frames; pro
/// Modul-Level recurse wir ueber 3 Funktionen → ~750 sichere Frames).
pub const MAX_MODULE_NESTING_DEPTH: usize = 256;

/// Top-Level: konvertiere CST-Wurzel zu [`Specification`].
///
/// # Errors
/// [`BuilderError`] wenn die CST-Struktur nicht der erwarteten
/// Grammar entspricht (sollte bei korrekt gebautem CST nicht
/// auftreten).
...
/// Sucht rekursiv nach einem `Keyword(kw)`-Token im gesamten Subtree.
/// Notwendig fuer Productions wie value_inheritance_spec, deren
/// Keyword-Tokens innerhalb von Repeat/Choice-Wrappern sitzen.
///
...
/// Findet den ersten Identifier in den Children eines Nodes — entweder
/// als direktes `Ident`-Token oder via `<identifier>`-Production-Wrapper.
...
/// Cons-rekursive Liste flach machen.
///
/// Annahme: Die Production hat Alternativen der Form
/// `single ::= item` und `cons ::= list separator? item`. Die
/// Recursion ist immer im *ersten* Child.
...
/// zerodds-lint: recursion-depth 64 (Template-Module-Body kann definitions rekursiv enthalten; bounded by IDL nesting)
fn build_definition(node: &CstNode<'_>, depth: usize) -> BuildResult<Definition> {
    // <definition> hat 7 Alternativen, jede beginnt mit
    // annotation_appl_seq + inner_dcl + ";".
    let annotations = build_annotation_seq(node)?;
    // Innere Production finden — eine der bekannten Decls.
    for c in &node.children {
        match c.production() {
    ...
    // Fallback fuer Delta-eingefuegte Konstrukte (T6.5): wenn keine
    // bekannte Production matchte, capture die innerste Internal-Child-
    // Production als VendorExtension. Bleibt tolerant, ohne dass jeder
    // Delta einen dedizierten AST-Builder mitbringt.
...
fn build_type_dcl(node: &CstNode<'_>) -> BuildResult<TypeDecl> {
    if let Some(c) = first_internal_child(node, ID_CONSTR_TYPE_DCL) {
        ...
        Err(BuilderError::new(
            "type_dcl without constr/typedef",
            node.span,
        ))
    }
}
...
fn build_enum_dcl(node: &CstNode<'_>) -> BuildResult<EnumDef> {
    ...
    // In der Praxis nie erreicht (4.3 Mrd Werte), aber Spec-konformer Check.
...
/// `<positive_int_const>` wickelt `<integer_literal>` ein.
...
fn build_bitmask_dcl(node: &CstNode<'_>) -> BuildResult<BitmaskDecl> {
    ...
    if let Some(list) = first_internal_child(node, ID_BIT_VALUE_LIST) {
        // bit_value_list: cons-style ohne explizite Item-Production —
        // jeder Eintrag ist ein annotation_appl_seq + identifier.
        // Walking: tiefster left-most ist erstes Element.
...
fn collect_bit_values(list: &CstNode<'_>, out: &mut Vec<BitValue>) -> BuildResult<()> {
    ...
    if first.production() == list.production() {
        ...
        // base case: erster annotation_appl_seq + ident
        out.push(extract_bit_value(list)?);
        return Ok(());
    }
    // cons case: Rest ist "," annotation_appl_seq <ident>
...
fn build_typedef_dcl(node: &CstNode<'_>) -> BuildResult<TypedefDecl> {
    ...
    // type_declarator hat zwei Alternativen: simple_type_spec oder
    // template_type_spec (jeweils + any_declarators). KEIN type_spec-Wrapper.
...
fn build_const_expr(node: &CstNode<'_>) -> BuildResult<ConstExpr> {
    // const_expr ::= or_expr — passe einfach durch.
...
fn build_shift_expr(node: &CstNode<'_>) -> BuildResult<ConstExpr> {
    // shift_expr hat ZWEI Operatoren (>> und <<). Spezialfall.
...
fn build_literal(node: &CstNode<'_>) -> BuildResult<Literal> {
    // <literal> wickelt einen Sub-Production-Wrapper (z.B.
    // <integer_literal>) ein, der wiederum den eigentlichen
    // Token-Leaf traegt. Boolean ist Spezialfall: Keyword TRUE/FALSE.
...
// Generic helper für left-assoziative single-op binary chains.
/// zerodds-lint: recursion-depth 64 (Parser/AST-Walk; bounded by IDL nesting)
...
/// Liefert `Vec<Export>` weil §7.4.3.1 multi-name attr_dcl mehrere
/// AttrDecl-Exports aus einer einzigen `attribute long a, b, c;`-Zeile
/// erzeugen kann. Andere Export-Alternativen liefern stets genau einen
/// Eintrag.
...
fn build_op_dcl(node: &CstNode<'_>) -> BuildResult<OpDecl> {
    ...
    // Identifier: erstes Ident-Token *nach* op_type_spec.
    let (text, span) = node
        .children
        .iter()
...
/// Liefert `Vec<AttrDecl>` weil §7.4.3.1 Rule 91/93 Multi-Name erlauben:
/// `attribute long a, b, c;` ist semantisch äquivalent zu drei
/// einzelnen Attribute-Deklarationen mit gemeinsamem Type/Readonly/
/// Annotations/Raises.
...
fn build_attr_spec(spec: &CstNode<'_>, outer_span: Span) -> BuildResult<Vec<AttrDecl>> {
    ...
    // attr_raises_expr kann unter dem attr_declarator-Knoten oder weiter
    // unten in einem `_alt_X`-Wrapper liegen — wir suchen rekursiv.
...
/// Sammelt alle `simple_declarator`-Knoten in Source-Reihenfolge.
///
/// Rule 91/93 nutzen `Symbol::Repeat(ZeroOrMore, [",", simple_declarator])`,
/// das im Compile-Pass zu einer synthetisierten `_rep_X`-Production
/// expandiert wird. Die Wiederholungs-simple_declarators sitzen daher
/// nicht direkt unter dem `*_attr_declarator`, sondern in einem nested
/// `_rep_X`-Wrapper. Wir walken deshalb alle internen Children und
/// sammeln rekursiv.
...
fn collect_exception_list(excep_node: &CstNode<'_>) -> BuildResult<Vec<ScopedName>> {
    // exception_list kann unter `_alt_X`-Wrappern verschachtelt sein.
...
/// Tiefen-Suche nach erster Production gegebener ID. Anders als
/// [`first_internal_child`] geht sie durch synthetisierte `_rep_X` /
/// `_alt_X` / `_chc_X`-Wrapper, die der Compile-Pass von
/// `Symbol::Repeat`/`Choice` erzeugt.
///
/// zerodds-lint: recursion-depth 64 (CST-Walk; bounded by IDL nesting)
...
fn build_annotation_dcl(node: &CstNode<'_>) -> BuildResult<AnnotationDcl> {
    ...
    if let Some(body) = first_internal_child(node, ID_ANNOTATION_BODY) {
        // body enthaelt eine `_rep_X`-synthetisierte Production mit
        // mehreren `annotation_body_member`-Knoten als Children. Wir
        // sammeln rekursiv alle annotation_body_member.
...
fn build_annotation_appl(node: &CstNode<'_>) -> BuildResult<Annotation> {
    let name_node = require_internal_child(node, ID_SCOPED_NAME, "scoped_name")?;
    let name = build_scoped_name(name_node)?;
...
/// Extrahiert den unescapten Wert aus einem `<string_literal>`-Wrapper —
/// strippt umschliessende Anfuehrungszeichen.
...
/// Liefert den ersten Identifier *nach* dem ersten Vorkommen einer
/// Marker-Production. Praktisch fuer Productions wie `provides
...
fn build_home_dcl(node: &CstNode<'_>) -> BuildResult<HomeDcl> {
    ...
    let supports = collect_supported_interfaces(header)?;
    // `manages <scoped_name>` — der Manage-Scoped-Name kommt als zweiter
    // <scoped_name>-Knoten im Header (erster ist im optionalen Inheritance/
    // Supports). Wir suchen den `manages`-Keyword-Index und nehmen den
    // ersten ScopedName *danach*.
...
fn find_manages_scoped_name(header: &CstNode<'_>) -> BuildResult<ScopedName> {
    // `manages` kommt als Keyword-Token im Header. ScopedName direkt
    // danach (nicht via Inheritance/Supports).
...
fn build_porttype_dcl(node: &CstNode<'_>) -> BuildResult<PorttypeDcl> {
    ...
    // Forward-Variante hat kein body.
    ...
    for c in collect_descendants(node, ID_ATTR_DCL) {
        // attr-Exporte unter port_export werden ueber descendants
        // erfasst; wir filtern hier nur jene, die direkt im PORT_BODY
        // sitzen (nicht in geschachtelten Konstrukten).
        body.extend(
            build_attr_dcl(c)?
                .into_iter()
                .map(attr_decl_to_dcl)
...
/// zerodds-lint: recursion-depth 64 (Template-Module-Body kann weitere template_module_dcls enthalten; bounded by IDL nesting)
fn build_template_module_dcl(node: &CstNode<'_>, depth: usize) -> BuildResult<TemplateModuleDcl> {
    ...
    // Body sind <tpl_definition>+; jede tpl_definition wraps either
    // <definition> oder <template_module_ref>. Fuer die AST-Foundation
    // walken wir nur die <definition>-Children.
    ...
    for tpl in collect_descendants(node, ID_TPL_DEFINITION) {
        ...
        // template_module_ref-Alias-Konstrukte ueberspringen wir hier;
        // sie werden vom Validator-Pass separat behandelt.
    }
...
fn build_template_module_inst(node: &CstNode<'_>) -> BuildResult<TemplateModuleInst> {
    ...
    // Letzter Identifier ist der Instance-Name. Spec §7.4.12 Rule 189:
...
fn build_actual_parameter(node: &CstNode<'_>) -> BuildResult<ConstExpr> {
    ...
    if let Some(ts) = first_internal_child(node, ID_TYPE_SPEC) {
        // Spec §7.4.12 Rule 191 erlaubt sowohl type_spec als auch
        // const_expr als actual_parameter. ScopedName-Pfad: Vendor-
        // Reference auf existierenden Typ. Primitive-Pfad (`long`,
        // `string`, ...): wir synthetisieren einen ScopedName aus dem
        // Keyword-Tokenset, damit der AST-Knoten ein einheitliches
        // Ziel hat. Der Validator-Pass kann das spaeter discrimieren.
...
/// Sammelt alle Keyword-Token im Subtree als pseudo-Identifier — Hilfe
/// fuer Konstrukte, die einen primitive Type-Spec wie `long`/`unsigned long`
/// in eine Identifier-Liste konvertieren wollen.
///
/// zerodds-lint: recursion-depth 64 (CST-Walk; bounded by IDL nesting)
...
/// Adapter: volle [`AttrDecl`] (Interface-Pipeline) → schlanke
/// [`AttrDcl`] (Component/Home/Connector). Drop von Raises/Annotations
/// — diese Felder sind in der Component-Pipeline ausserhalb des AST
/// (Component-Validatoren in §7.4.8.4 nutzen die schlanke Form).
...
// ============================================================================
// Annotations-Helfer fuer neue Definition-Variants
// ============================================================================
...
mod tests {
    ...
    fn builds_readonly_attr_with_raises() {
        // §7.4.3.1 Rule 91 alt-1: readonly attribute mit raises_expr.
        let ast = parse_to_ast(
    ...
    // ========================================================================
    // K1-Followup: Builder-Routing-Tests fuer 11 Definition-Variants
    // ========================================================================
    ...
    // ========================================================================
    // K1.9 Coverage-Audit: fehlende Tests fuer §7.4.10/11 r158/r182
    // ========================================================================
