...
pub struct BridgeMetrics {
    ...
    /// MQTT → DDS Samples written.
    pub dds_samples_in_total: Arc<Counter>,
    /// DDS → MQTT Samples published.
...
impl BridgeMetrics {
    /// Registriert den Standard-Set.
    pub fn register(registry: &Registry) -> Self {
        registry.set_help(
            "zerodds_mqtt_frames_in_total",
            "MQTT frames received from broker",
        );
        registry.set_help(
            "zerodds_mqtt_frames_out_total",
            "MQTT frames sent to broker",
        );
        registry.set_help("zerodds_mqtt_bytes_in_total", "MQTT bytes received");
        registry.set_help("zerodds_mqtt_bytes_out_total", "MQTT bytes sent");
        registry.set_help(
            "zerodds_mqtt_connections_active",
            "Active broker connections",
        );
        registry.set_help(
            "zerodds_mqtt_connections_total",
            "Lifetime broker connect attempts",
        );
        registry.set_help(
            "zerodds_mqtt_dds_samples_in_total",
            "Samples written into DDS via MQTT",
        );
        registry.set_help(
            "zerodds_mqtt_dds_samples_out_total",
            "Samples published from DDS to MQTT",
        );
        registry.set_help("zerodds_mqtt_errors_total", "Wire/codec/socket errors");
        Self {
            frames_in_total: registry.counter("zerodds_mqtt_frames_in_total", Labels::new()),
            frames_out_total: registry.counter("zerodds_mqtt_frames_out_total", Labels::new()),
            bytes_in_total: registry.counter("zerodds_mqtt_bytes_in_total", Labels::new()),
            bytes_out_total: registry.counter("zerodds_mqtt_bytes_out_total", Labels::new()),
            connections_active: registry.gauge("zerodds_mqtt_connections_active", Labels::new()),
            connections_total: registry.counter("zerodds_mqtt_connections_total", Labels::new()),
            dds_samples_in_total: registry
                .counter("zerodds_mqtt_dds_samples_in_total", Labels::new()),
            dds_samples_out_total: registry
                .counter("zerodds_mqtt_dds_samples_out_total", Labels::new()),
            errors_total: registry.counter("zerodds_mqtt_errors_total", Labels::new()),
        }
    }
}
...
/// Installiert SIGTERM/SIGINT/SIGHUP-Watcher.
#[cfg(unix)]
pub fn install_signal_watcher(
    ...
    thread::Builder::new()
        .name("zerodds-mqtt-signals".into())
        .spawn(move || {
...
// ============================================================================
// A5 — Catalog/Healthz/Metrics-Endpoint (§5.2).
// ============================================================================
...
impl CatalogSnapshot {
    ...
    pub fn render_json(&self) -> String {
        ...
        for (i, t) in self.topics.iter().enumerate() {
            ...
            out.push_str("\",\"mqtt_topic\":\"");
            push_json_str(&mut out, &t.mqtt_topic);
            out.push_str("\",\"direction\":\"");
            push_json_str(&mut out, &t.direction);
            out.push_str("\",\"qos\":{\"mqtt_qos\":");
            out.push_str(&t.mqtt_qos.to_string());
...
mod tests {
    ...
    #[test]
    fn metrics_visible_in_prometheus_render() {
        let r = Registry::new();
        let m = BridgeMetrics::register(&r);
        m.dds_samples_in_total.add(3);
        let text = r.render_prometheus();
        assert!(text.contains("zerodds_mqtt_dds_samples_in_total 3"));
    }
    ...
    fn catalog_render_json_contains_topic_fields() {
        ...
        let s = CatalogSnapshot::from_config(&cfg);
        let j = s.render_json();
        assert!(j.contains("\"service\":\"zerodds-mqtt-bridged\""));
        assert!(j.contains("\"dds_name\":\"Sensor\""));
        assert!(j.contains("\"mqtt_topic\":\"sensors/data\""));
        assert!(j.contains("\"mqtt_qos\":1"));
    }

    #[test]
    fn admin_endpoint_serves_paths() {
        use std::io::{Read, Write};
        use std::net::TcpStream;
        let cfg = DaemonConfig::default_for_dev();
        let snap = Arc::new(CatalogSnapshot::from_config(&cfg));
        let reg = Arc::new(Registry::new());
        let _m = BridgeMetrics::register(&reg);
        ...
        assert!(body.contains("zerodds-mqtt-bridged"));
