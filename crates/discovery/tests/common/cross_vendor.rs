...
//! run on the Linux bench host via `ssh llvm@llvm`. The structs hold
//! the local child handle plus a Drop impl that kills the remote
//! process via `pkill` — otherwise `fastdds shape` / `ddsperf`
//! processes are left running on the bench host.
//!
//! # Type strategy
//!
//! `RemoteSubprocess` is a generic wrapper: the `pkill_pattern`
//! identifies the remote program, the local `Child` holds the SSH
//! session. Without an SSH key, the lab password is piped via
//! `sshpass` — acceptable for a lab setup, to be replaced by key auth
//! in production.
//!
//! # `#[ignore]` strategy
//!
//! All live tests are marked `#[ignore]`. Locally:
//!
//! ```bash
...
//! ```
//!
//! If you can't reach the lab, see `live_host_available()` — as soon
//! as the env var `LLVM_HOST_AVAILABLE=1` is not set AND `sshpass` or
//! the host connection is missing, the helpers skip with a clear error
//! message.
...
/// Check whether the live host is reachable. Tests without a lab can
/// skip early this way, before SSH timeouts bleed the test out.
pub fn live_host_available() -> bool {
    if std::env::var("LLVM_HOST_AVAILABLE").is_ok() {
        return true;
    }
    // Fallback: is `sshpass` present locally? Without `sshpass` all
...
/// Generic SSH subprocess wrapper. `pkill_pattern` is the `pkill -f`
/// argument that `Drop` uses to kill the remote process.
...
impl RemoteSubprocess {
    /// Spawns `cmd` over `sshpass+ssh` on the bench host. `pkill_pattern`
    /// matches the remote process (regex-free, used as a substring with
    /// `pkill -f`).
...
/// Starts `fastdds shape publisher` on the bench host.
///
/// We set `--domain` explicitly; the topic is one of the three shape
/// topics (`Square`, `Triangle`, `Circle`) per FastDDS convention.
/// Color `RED` as default, because that is the first available one for
/// the `fastdds shape` default profile.
...
/// Starts `fastdds shape subscriber` on the bench host.
...
/// (cyclone_typelookup_sub_side). `-D` is duration in seconds, `-i` is
/// the domain ID (see memory `reference_ddsperf_flags` — mind the flag
/// trap).
...
/// Detect the local LAN IP — on macOS via `ipconfig`, on Linux via
/// `hostname -I`. Needed for SPDP beacons (Cyclone/FastDDS send unicast
/// replies to the locator announced in the beacon).
