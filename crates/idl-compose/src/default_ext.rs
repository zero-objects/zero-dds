...
//! Alias-Annotation. Damit sehen alle 17 Backends denselben
...
pub enum DefaultExt {
    /// `@final` — no evolution allowed (Cyclone DDS default via `-x`).
    Final,
    /// `@appendable` — members may be appended (FastDDS/spec-leaning
    /// default).
    Appendable,
    /// `@mutable` — full XTypes member-level evolution.
    Mutable,
}
