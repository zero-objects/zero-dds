...
fn bit_bound_selects_enum_wire_width() {
    ...
    assert!(
        src.contains("Wire.put_u8 w (small_to_int"),
        "1-byte enum encode missing:\n{src}"
    );
    assert!(
        src.contains("Wire.put_u16 w (med_to_int"),
        "2-byte enum encode missing:\n{src}"
    );
    assert!(
        src.contains("Wire.put_u32 w (def_to_int"),
        "4-byte enum encode missing:\n{src}"
    );
    // Decode: matching narrow reader.
    assert!(
        src.contains("small_of_int (Wire.get_u8 r)"),
        "1-byte enum decode missing:\n{src}"
    );
    assert!(
        src.contains("med_of_int (Wire.get_u16 r)"),
        "2-byte enum decode missing:\n{src}"
    );
    assert!(
        src.contains("def_of_int (Wire.get_u32 r)"),
