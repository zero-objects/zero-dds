...
- [Language reference](language.md) — types, modules, struct,
  union, enum, typedef, constant.
- [Annotations](annotations.md) — `@key`, `@id`, `@final`,
  `@appendable`, `@mutable`, `@hashid`, `@verbatim`, …
- [Codegen with `zerodds-idlc`](codegen.md) — invocation, language
  flags, output layout (conceptual overview).
- [`zerodds-idlc` handbook](idlc-handbook.md) — comprehensive
  end-user reference: install, CLI, build integration, cookbook,
  troubleshooting.
- [CDR wire format](cdr-wire-format.md) — XCDR1 / XCDR2 byte form
  for every IDL construct, alignment rules, debugging tips.
- [Examples](examples/) — a handful of typical schemas.
...
- [Language reference](language.md) for the syntax in depth.
- [Codegen](codegen.md) for the `zerodds-idlc` CLI.
- [05 Integration](../05-integration/README.md) to wire the
