...
zunächst als "done — alternative Implementations-Wahl" oder
...
- ✅ **`valuetype`** — DONE 2026-04-28 (cpp class+factory,
  ...
  Abstract+Concrete; alle 9 Tests grün).
...
- `idl4-java-1.0.md`: nur `fixed`/`any`/`valuetype`/non-service-
  interface partial; bitset/bitmask schon länger live.
...
Pointer-Type für Sharing zwischen Instances.
...
**Auflösung:** `crates/java-omgdds/java/` als echtes Maven-Projekt
...
Encoder + UTF-8-String + Alignment-Cap=4). 18 JUnit-Tests grün via
`mvn test`. Open-Items C/D/F als Foundation done; E (Dynamic-Types)
als partial. Pure-Java-RTPS-Stack für Cross-Process-Interop bleibt
...
**Begründung Korrektur:** Die Pure-Java-Implementation-Realisierung ist **nicht**
Spec-konform abnahmefähig:
- JNI verlangt eine native Lib, die **pro Java-Ziel-Plattform** gebaut
  werden muss (Linux x86_64, macOS aarch64, Windows x86_64, ARM, ...).
- Wir können auf den Build-Hosts unserer Anwender keinen Rust-
...
Pure-Java-Implementation — als Tooling für Code, der parallel mit Rust-Anwendungen
laufen will, weiterhin nützlich, aber **nicht** als Spec-Java-PSM-
Realisierung.

**Re-Audit-Trigger:** Wenn Java-PSM-Sprint startet. Schätzaufwand:
groß (kompletter DCPS-Stack in Java; alternativ: Java→XCDR→Rust-
Side-Cars-Approach).

**Audit-Items, die zurückgestuft werden:**
- `zerodds-java-psm-1.0.md` §1.1, §1.2, §2.0-§2.4: von `done — Pure-Java-Implementation`
  auf `open — Native Java-PSM nötig`.
...
**Begründung Korrektur:** Wire-Form-Compliance allein erfüllt §7.2.2
nicht. Spec verlangt:
...
**Begründung Korrektur:** Per-Application-JAR-Approach scheitert bei
Dynamic Types:
- Anwendung A definiert TopicType `Foo` zur Compile-Zeit.
- Discovery liefert TopicType `Bar` zur Laufzeit (vorher unbekannt).
- Per-Application-JAR enthält nur `Foo`-Wrapper — `Bar` kann nicht
  instantiiert werden.

Single-Universal `omgdds.jar` enthält:
- Volles DDS-DCPS-API in `org.omg.dds.*`.
- DynamicType + DynamicData für Runtime-Type-Erzeugung
  (XTypes-Pfad, siehe `crates/xtypes/`).

**Scope:** Folgt aus C+D. JAR ist conceptually unabhängig von
...
**Begründung Korrektur:** "JNI weiß nicht wann disposed werden
darf" entfällt mit Native Java — Java-GC kann mit `Cleaner`
(Java 9+) oder `PhantomReference` Auto-Close korrekt implementieren.

**Scope:** Folgt aus C. `Cleaner.register(...)`-basierter Auto-Close
für alle Reference-Type-Entities, sobald Native-Java-PSM da ist.
...
## H) Sekundär-Kunden Spec-Coverage (neu 2026-04-28)
...
für get_applications. 44 Tests grün. Audit
`docs/spec-coverage/dds-web-1.0.md` — 16 done / 0 partial / 0 open /
2 n/a (§3-§6 Glossar/Acks + §8.4 SOAP-Platform).
...
Branch). 38 Tests grün. Audit `docs/spec-coverage/dds-tsn-1.0.md`
...
ist VOLL ABGEDECKT über `crates/xml/src/qos.rs`. CCM-Component-
Mapping (§7) bleibt Phase-2 (out-of-scope).

**Spec:** `docs/standards/cache/omg/dds4ccm-1.1.pdf`.

**Begründung:** Schon Cross-Ref aus dds-psm-cxx §7.6.2.1 +
zerodds-java-psm §3.1.2 (XML-QoS-Profile-Quelle); eigener Audit
fehlte. Volle Coverage benötigt CCM-Component-Bridge für
...
- XML-QoS-Loader ist live (`crates/xml/src/qos.rs`); Component-
  Mapping ist erst nötig wenn ein CCM-Container produktiv wird.
...
Topic-Name, SampleVariable mit value_rank=-1 für Scalar). 33 Tests
grün. Audit `docs/spec-coverage/dds-opcua-1.0.md` mit 8 done /
...
**Auflösung Foundation:**

- `crates/idl-ts/` neu — IDL → TypeScript-Codegen (5 Tests grün).
...
**Verbleibend `open`:**
- WASM-Runtime (`crates/dcps-wasm/`) für Browser.
...
Siehe Punkt N unten für die Auflösung.
...
**Heruntergeladen + auditiert (Punkte H1-H4):** dds-web-1.0,
dds-tsn-1.0, dds4ccm-1.1, dds-opcua-1.0.
...
**Noch nicht recherchiert** (potentiell Sekundär-Relevant):
...
**Scope:** Time-Service-Spec; relevant für DDS-Security (Clock-
...
**Auflösung:** `crates/ccm/` neu — §6 Component Model
Equivalent-IDL-Transformation (Provides/Uses/Emits/Publishes/Consumes/
Attribute), Home-Equivalent (Explicit + Implicit + Equivalent für
...
grün. Audit-File `docs/spec-coverage/omg-ccm-4.0.md` mit 25 done /
...
**Auflösung:** `crates/rtc/` neu — Lightweight RTC, ExecutionContext,
...
(§6.3) als Implementation-Form. 37 Tests grün. Audit
...
**Auflösung:** `crates/coap-bridge/` neu — voller Wire-Codec
...
Code-Registry §12.1 + Klassifikations-Prädikate. 34 Tests grün
...
/ 16 n/a (Reliability/DTLS/Multicast/URI außerhalb Codec-Scope).

### M2) WebSocket Protocol (RFC 6455) — DONE 2026-04-28

**Datei:** `docs/standards/cache/ietf/rfc6455.txt`.

**Auflösung:** `crates/websocket-bridge/` neu — voller Base-Framing-
...
Frame-Konstruktoren. 32 Tests grün. Audit
...
(HTTP-Handshake/State-Machine/Extensions außerhalb Codec-Scope).

### M3) MQTT 5.0 (OASIS) — DONE 2026-04-28

**Datei:** `docs/standards/cache/oasis/mqtt-5.0.html`.

**Auflösung:** `crates/mqtt-bridge/` neu — voller Wire-Codec:
...
Enforcement). 36 Tests grün. Audit
...
**Auflösung:** `crates/amqp-bridge/` neu — Type-System §1.6 Subset
...
AMQP/SASL/Reserved-Frame-Type). 27 Tests grün. Audit
...
Sections — alle Caller-Layer über dem Type-System) / 3 n/a (Overview
...
**Auflösung:** `crates/ros2-rmw/` neu — Topic-Name-Mangling
...
SYSTEM_DEFAULT). 23 Tests grün. Audit
...
**Auflösung:** `crates/grpc-bridge/` neu — Length-Prefixed-Message
...
mit numeric/from_numeric/name screaming-snake-case + is_ok-Prädikat).
32 Tests grün. Audit `docs/spec-coverage/grpc-protocol.md` mit
...
**Auflösung:** `crates/transport-tcp/` existiert bereits voll
...
**Status:** Aufgelöst 2026-04-28 (separater Commit, siehe
...
10. **M5) ROS 2 RMW** — "ROS2 ist das womit alle werbung machen,
    wir müssen da mitziehen" (User). Eigener Sprint
    `crates/rmw-zerodds/`.
11. **M6) gRPC-Bridge** — "wie websockets etwas was uns abhebt"
    (User). Cloud-Bridge.
12. **M1) CoAP-Bridge** — IoT-Sensor-Bridge, Sensor-Sekundärkunden.
...
19. **H2) DDS-TSN** — abhängig von TSN-Hardware-Lab.
20. **C/D/E/F) Native Java-PSM** — größter Brocken; eigener
    Sprint `crates/java-omgdds/`.

---

*Dieses Dokument ist die Single-Source-of-Truth für alle K10-K15-
Items, die Spec-konform aber nicht spec-vollständig waren. Audit-
