...
**Quelle:** `docs/specs/zerodds-xcdr2-d-1.0.md` — ZeroDDS D XCDR2 TypeSupport-Codegen-Spec.
...
**Repo:** `endpoints/d/zerodds.d` (Writer, Reader inkl. getU8/getU16/getU64/getF32/getString/getSeqU8 (byte-exakter Inverse)); `crates/idl-d/src/emitter.rs` (marshalXCDR).

**Tests:** `endpoints/d/zerodds.d` test.d (byte identity + sync + async), `endpoints/d/example_sync|async.d` (voller Feld-Decode), `crates/idl-d/tests/golden.rs`.
...
**Repo:** `map_get` (Invers von `map_type`) + `Reader`-Wire-Core im Prelude + freie `unmarshalFrom{ty}`/`UnmarshalXCDR{ty}` je struct+union (`crates/idl-d/src/emitter.rs`).
...
**Decision-Record:** Decode-codegen deckt jeden Feldtyp ab (prim/string/seq/enum/typedef/nested/array/union/map/@mutable/wchar/wstring/long double). @final=inline, @appendable=DHEADER-skip, @mutable=DHEADER + je Member EMHEADER+NEXTINT skip.
...
**Spec:** §5 — bool/octet/char/short..long long/float/double/string/sequence<octet> mit exaktem Wire-Layout; signed via *cast(uint*)&v.
...
**Repo:** `crates/idl-d/src/emitter.rs`: `marshalInto(ref Writer)` je struct + `map_type` Scoped-struct + `map_sequence` struct-Element (Collection-DHEADER + count + per-element marshalInto; kollisionssichere Var-Namen zdBody/zdSub/zdElem).

**Tests:** `crates/idl-d/tests/golden.rs::nested_struct_emits_marshal_into` + `nested_is_byte_identical_vs_rust_golden` (`gdc`, byte-identisch gegen `golden_nested_le/be.bin`).
...
**Repo:** `crates/idl-d/src/emitter.rs::collect_typedefs`/`resolve_typedef` — Alias-Kette (inkl. `sequence`-Elemente) wird vor `map_type` zum Grundtyp aufgelöst.

**Tests:** `crates/idl-d/tests/golden.rs::typedef_resolves_to_underlying_type` + `typedef_is_byte_identical_vs_rust_golden` (byte-identisch gegen `golden_typedef_le/be.bin`).
...
**Repo:** `crates/idl-d/src/emitter.rs`: `Declarator::Array`-Zweig — `array_size` wertet den Bound aus, `build_array_put` verschachtelt den Element-Put in row-major-Loops; Feldtyp wird das sprachnative Array.

**Tests:** `crates/idl-d/tests/golden.rs::array_*` (byte-identisch gegen `golden_array_le/be.bin`: `long xs[3]` + `short m[2][2]` + `octet bs[4]`).
...
**Repo:** `crates/idl-d/src/emitter.rs::emit_union` — Holder mit Diskriminator + einem Feld je Case-Member; `marshalInto` putet den Diskriminator, dann dispatcht ein `switch`/`case`/`match` auf das selektierte Member. Integer-Family-Diskriminator + Integer-Labels.

**Tests:** `crates/idl-d/tests/golden.rs::union_*` (byte-identisch gegen `golden_union_le/be.bin`: disc=2 selektiert `unsigned short b` — prüft Nicht-First-Case-Dispatch).
...
**Repo:** `crates/idl-d/src/emitter.rs` — Map-Member im nativen assoziativen Idiom + Key-Sortierung vor dem Marshalling; primitiv-Paar-Regel für den Collection-DHEADER.

**Tests:** `crates/idl-d/tests/golden.rs::map_*` (byte-identisch gegen `golden_map_le/be.bin`: `map<long, unsigned long>` {1,2}).
...
**Repo:** `crates/idl-d/src/emitter.rs` — `putWString`/`put_wstring` (manuelles UTF-16 mit Surrogatpaaren) + `wchar`→`putU32`. Wire-Core im Prelude.

**Tests:** `crates/idl-d/tests/golden.rs::wide_is_byte_identical_vs_rust_golden` (byte-identisch gegen `golden_wide_le/be.bin`: c=U+03A9, s="wπ").
...
**Repo:** `crates/idl-d/src/emitter.rs` — `putLongDouble`/`put_long_double`: binary128 durch exaktes Widening des `float64`-Werts (Sign + 15-Bit-Exponent + 112-Bit-Mantisse), endian-korrekt.

**Tests:** `crates/idl-d/tests/golden.rs::longdouble_is_byte_identical_vs_rust_golden` (byte-identisch gegen `golden_longdouble_le/be.bin`: d=1.1).
...
**Repo:** `crates/idl-d/src/emitter.rs` — @mutable-`marshalInto`: DHEADER-gerahmte Member-Liste, je Member EMHEADER (LC4 = `0x40000000 | member-id`) + NEXTINT (Body-Länge) + Body (in Sub-Writer serialisiert). Member-ids aus `@id(n)` oder sequenziell.

**Tests:** `crates/idl-d/tests/golden.rs::mutable_*` (byte-identisch gegen `golden_mutable_le/be.bin`).
...
**Repo:** `endpoints/d/zerodds.d` (Writer im Big-Mode liefert die BE-Serialisierung).
...
**Repo:** `crates/idl-d/src/emitter.rs` — Structs mit `@key`-Membern erhalten eine `keyHash`/`Key_Hash`-Methode: die `@key`-Member PLAIN_CDR2-BE serialisiert (member-id-Reihenfolge), ≤16-Byte-Key-Holder auf 16 Byte zero-gepaddet, größere (oder dynamisch dimensionierte) via MD5(bytes)[0..16]. Die statische max-Key-Size-Analyse ist im geteilten `zerodds_idl::keyhash`.

**Tests:** `crates/idl-d/tests/golden.rs::keyhash_is_byte_identical_vs_rust_golden` (byte-identisch gegen `golden_keyhash.bin` via `zerodds_cdr::compute_key_hash`).
...
### §8 `endpoints/d` als Referenz-Writer/Reader
...
**Tests:** CI-Job `endpoints-d` führt beide aus (`make example_sync` + `make example_async`).
...
Test-Lauf: `GOLDEN_DIR=<gold> cargo test -p zerodds-idl-d` — 29 Tests grün, 0 failed (codepit, Toolchain `gdc`).
