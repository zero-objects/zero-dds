...
**Repo:** `crates/idl-csharp/src/` (union-, `map<>`-, Array-, Nested-Struct-, `wchar`/`wstring`/`long double`-Emit-Arme).

**Tests:** —

**Status:** done

**Decision-Record:** Das `idl-csharp`-Codegen emittiert diese Konstrukte (Unions, `map<>`, Arrays, Nested-Struct-Member, `wchar`, `wstring`, `long double`). Nur `@mutable`-Unions bleiben zurückgestellt (siehe §6.a).
...
**Repo:** `crates/idl-csharp/src/` (`@mutable` PL_CDR2-EMHEADER- / PL_CDR1-Pfade).
