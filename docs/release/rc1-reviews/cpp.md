...
C++17-RAII-Wrapper-Header (`dds/dds.hpp`) ueber das `zerodds-c-api`-Interface; voll spec-konform mit DDS-PSM-Cxx 1.0 §7.5.
...
Siehe `crates/cpp/README.md` + `CHANGELOG.md` + `src/`.
...
- DDS-PSM-Cxx 1.0 voll: alle Spec-Klassen unter `include/dds/`
...
- Kohärenz: (a) wired über C-FFI, (b) Konsumenten in Cross-Language-
  Bindings, (c) getestet mit 1 cargo-test (compile+link+10 sub-asserts)

## 8 Tests + Lints + Doc-Build

- `cargo test -p cpp`: 1 cargo-test (compile+link+10 sub-asserts)
