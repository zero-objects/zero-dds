...
.zddsrec Record/Replay-Format fuer Wire-Frames + Discovery-Snapshot.
...
Siehe `crates/recorder/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 17 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-recorder`: 17 Tests
