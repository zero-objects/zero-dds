...
X.509-Validator + RSA-PSS Verify + CRL/OCSP-Status.
...
Siehe `crates/security-pki/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 197 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-pki`: 197 Tests
