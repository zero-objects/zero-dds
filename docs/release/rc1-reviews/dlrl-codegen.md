# RC1 Review — `zerodds-dlrl-codegen`
...
DLRL-Codegen — generiert DLRL-Object-Klassen aus DLRL-Pragmas im IDL.
...
Siehe `crates/dlrl-codegen/README.md` + `CHANGELOG.md` + `src/`.
...
DLRL 1.2 §B.6 IDL-Pragma-Subset; integriert mit `zerodds-dlrl`.
...
- Kohärenz: (a) wired, (b) Konsumenten in Cross-Crate-Tests, (c)
  getestet mit 15 tests

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-dlrl-codegen`: 15 tests
- `cargo clippy -p zerodds-dlrl-codegen --tests -- -D warnings`: clean
