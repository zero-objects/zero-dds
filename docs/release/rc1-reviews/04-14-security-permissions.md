...
Permissions-XML-Parser + Topic/Partition-ACL-Engine.
...
Siehe `crates/security-permissions/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 143 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-permissions`: 143 Tests
