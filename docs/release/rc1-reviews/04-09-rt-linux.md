...
Linux-RT-Scheduling Helper (SCHED_FIFO, mlockall, CPU-Affinity).
...
Siehe `crates/rt-linux/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 7 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-rt-linux`: 7 Tests
