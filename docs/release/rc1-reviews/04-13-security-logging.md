...
DDS-Security Audit-Log-Sink (RFC-5424 Severity, NDJSON-Wire).
...
Siehe `crates/security-logging/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 16 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-logging`: 16 Tests
