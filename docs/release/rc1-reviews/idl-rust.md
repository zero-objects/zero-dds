...
ZeroDDS-eigene Spec — IDL4 → Rust DataTypes mit DdsType-Trait.
...
- LOC: 1972 (11 src-Files)
- Public-Items: 26 (11 CONNECTED, 0 DEAD nach Layer-3-Pass-2)

### 3.4 Coherence-Audit (§1.5b)

| Family | Spec-Anker | External Production-Refs | Klassifikation | Decision |
|---|---|---|---|---|
| **Top-Level Codegen-API** (`generate_*_module`, `*GenOptions`, `Result`, `*GenError`) | ZeroDDS-eigene Spec — IDL4 → Rust DataTypes mit DdsType-Trait | tools/idlc + Snapshot-Tests | CONNECTED | — |
...
**Sweep-Verifikation (§1.5b Pass 2):** `/tmp/zerodds-audit/idl-rust.tsv`
zeigt 26 Public-Items, 0 DEAD. Alle Items entweder direkt CONNECTED
...
`cargo test -p zerodds-idl-rust` + `cargo clippy --all-targets -- -D warnings` + `cargo doc --no-deps` — alle ✅.
