...
Proc-Macro: leitet FlatStruct + FlatRead/FlatWrite aus repr(C) Strukturen ab.
...
Siehe `crates/flatdata-derive/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 0 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-flatdata-derive`: 0 Tests
