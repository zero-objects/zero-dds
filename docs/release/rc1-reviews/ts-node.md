...
TypeScript-Node-Binding via koffi-FFI gegen libzerodds. DDS-PSM-Cxx-style API parallel zur Legacy-Runtime-API.
...
Siehe `crates/ts-node/README.md` + `CHANGELOG.md` + `src/`.
...
- Listener via koffi-callback ist Phase-2 (siehe Vendor-Spec listener-callbacks §7.5 n/a)
...
- Kohärenz: (a) wired über C-FFI, (b) Konsumenten in Cross-Language-
  Bindings, (c) getestet mit 4 ts-tests (1 SKIP)

## 8 Tests + Lints + Doc-Build

- `cargo test -p ts-node`: 4 ts-tests (1 SKIP)
