...
ZeroDDS-eigene Spec — IDL4 → TypeScript (Browser + Node.js).
...
- LOC: 5036 (2 src-Files)
- Public-Items: 8 (2 CONNECTED, 0 DEAD nach Layer-3-Pass-2)

### 3.4 Coherence-Audit (§1.5b)

| Family | Spec-Anker | External Production-Refs | Klassifikation | Decision |
|---|---|---|---|---|
| **Top-Level Codegen-API** (`generate_*_module`, `*GenOptions`, `Result`, `*GenError`) | ZeroDDS-eigene Spec — IDL4 → TypeScript (Browser + Node.js) | tools/idlc + Snapshot-Tests | CONNECTED | — |
...
**Sweep-Verifikation (§1.5b Pass 2):** `/tmp/zerodds-audit/idl-ts.tsv`
zeigt 8 Public-Items, 0 DEAD. Alle Items entweder direkt CONNECTED
...
`cargo test -p zerodds-idl-ts` + `cargo clippy --all-targets -- -D warnings` + `cargo doc --no-deps` — alle ✅.
