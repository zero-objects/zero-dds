...
- LOC: 4759 (11 src-Files)
- Public-Items: 21 (4 CONNECTED, 0 DEAD nach Layer-3-Pass-2)

### 3.4 Coherence-Audit (§1.5b)

| Family | Spec-Anker | External Production-Refs | Klassifikation | Decision |
|---|---|---|---|---|
| **Top-Level Codegen-API** (`generate_*_module`, `*GenOptions`, `Result`, `*GenError`) | OMG IDL4-Java 1.0 (idl4-java-1.0) + DDS-Java-PSM 1.0 | tools/idlc + Snapshot-Tests | CONNECTED | — |
...
**Sweep-Verifikation (§1.5b Pass 2):** `/tmp/zerodds-audit/idl-java.tsv`
zeigt 21 Public-Items, 0 DEAD. Alle Items entweder direkt CONNECTED
...
`cargo test -p zerodds-idl-java` + `cargo clippy --all-targets -- -D warnings` + `cargo doc --no-deps` — alle ✅.
