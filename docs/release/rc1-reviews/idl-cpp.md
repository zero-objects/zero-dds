...
OMG IDL4-CPP 1.0 (idl4-cpp-1.0, formal/2018-07-01) + DDS-PSM-Cxx 1.0 + DDS-RPC C++ PSM.
...
- LOC: 5801 (13 src-Files)
- Public-Items: 30 (4 CONNECTED, 0 DEAD nach Layer-3-Pass-2)

### 3.4 Coherence-Audit (§1.5b)

| Family | Spec-Anker | External Production-Refs | Klassifikation | Decision |
|---|---|---|---|---|
| **Top-Level Codegen-API** (`generate_*_module`, `*GenOptions`, `Result`, `*GenError`) | OMG IDL4-CPP 1.0 (idl4-cpp-1.0, formal/2018-07-01) + DDS-PSM-Cxx 1.0 + DDS-RPC C++ PSM | tools/idlc + Snapshot-Tests | CONNECTED | — |
...
**Sweep-Verifikation (§1.5b Pass 2):** `/tmp/zerodds-audit/idl-cpp.tsv`
zeigt 30 Public-Items, 0 DEAD. Alle Items entweder direkt CONNECTED
...
`cargo test -p zerodds-idl-cpp` + `cargo clippy --all-targets -- -D warnings` + `cargo doc --no-deps` — alle ✅.
