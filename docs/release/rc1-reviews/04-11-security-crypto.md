...
AES-GCM/HMAC Crypto-Plugin mit HW-Accel-Detect (AES-NI/ARM-CE).
...
Siehe `crates/security-crypto/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 80 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-crypto`: 80 Tests
