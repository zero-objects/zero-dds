# RC1 Review — `zerodds-zerodds-xml-wire`
...
- LOC: 1477 (6 src-Files)
- Public-Items: 14 (14 CONNECTED, 0 DEAD nach Layer-3-Pass-2)

### 3.4 Coherence-Audit (§1.5b)

| Family | Spec-Anker | External Production-Refs | Klassifikation | Decision |
|---|---|---|---|---|
| **Top-Level Codegen-API** (`generate_*_module`, `*GenOptions`, `Result`, `*GenError`) | DDS-XML 1.0 Wire-PSM — XML↔CDR-Codec, XSD-Schema-Gen, Streaming-Parser | tools/idlc + Snapshot-Tests | CONNECTED | — |
...
**Sweep-Verifikation (§1.5b Pass 2):** `/tmp/zerodds-audit/zerodds-xml-wire.tsv`
zeigt 14 Public-Items, 0 DEAD. Alle Items entweder direkt CONNECTED
...
`cargo test -p zerodds-zerodds-xml-wire` + `cargo clippy --all-targets -- -D warnings` + `cargo doc --no-deps` — alle ✅.
