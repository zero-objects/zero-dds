# RC1 Review — `zerodds-opcua-gateway`
...
OPC UA ↔ DDS Gateway — bidirektionales Protocol-Mapping per OMG DDS-OPCUA 1.0.
...
Siehe `crates/opcua-gateway/README.md` + `CHANGELOG.md` + `src/`.
...
DDS-OPCUA 1.0: 14 done + 8 n/a auf 22 Items in `docs/spec-coverage/dds-opcua-1.0.md`.
...
- Kohärenz: (a) wired, (b) Konsumenten in Cross-Crate-Tests, (c)
  getestet mit 119 tests

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-opcua-gateway`: 119 tests
- `cargo clippy -p zerodds-opcua-gateway --tests -- -D warnings`: clean
