...
DDS-Web SOAP-PSM Bridge — exposes ZeroDDS via SOAP-Web-Service-Endpoints fuer Legacy-Web-Clients.
...
Siehe `crates/zerodds-soap/README.md` + `CHANGELOG.md` + `src/`.
...
DDS-Web 1.0 SOAP-Subset (siehe `docs/spec-coverage/dds-web-1.0.md`).
...
- Kohärenz: (a) wired, (b) Konsumenten in Cross-Crate-Tests, (c)
  getestet mit 37 tests

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-soap`: 37 tests
- `cargo clippy -p zerodds-soap --tests -- -D warnings`: clean
