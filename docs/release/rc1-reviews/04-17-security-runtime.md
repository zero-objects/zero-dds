...
Plugin-Runtime: Identity/Access/Crypto-Handle-Lifecycle, SPDP-Token-Wiring.
...
Siehe `crates/security-runtime/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 252 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-runtime`: 252 Tests
