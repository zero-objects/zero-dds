...
OpenTelemetry-OTLP-Sink fuer Logs/Metrics/Spans aus DCPS-Runtime.
...
Siehe `crates/observability-otlp/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 8 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-observability-otlp`: 8 Tests
