...
RTPS-Header-AAD + Submessage-Crypto-Wrap (DDS-Security 9.5).
...
Siehe `crates/security-rtps/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 31 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-security-rtps`: 31 Tests
