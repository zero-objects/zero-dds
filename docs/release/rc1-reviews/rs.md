...
Idiomatic Rust SDK-Facade. Re-exportiert dcps/aio/qos/types/monitor/recorder/foundation aus den darunterliegenden Crates.
...
Siehe `crates/rs/README.md` + `CHANGELOG.md` + `src/`.
...
- Facade-Crate; konsumiert direkt zerodds-dcps statt C-FFI
...
- Kohärenz: (a) wired über C-FFI, (b) Konsumenten in Cross-Language-
  Bindings, (c) getestet mit 3 cargo-tests

## 8 Tests + Lints + Doc-Build

- `cargo test -p rs`: 3 cargo-tests
