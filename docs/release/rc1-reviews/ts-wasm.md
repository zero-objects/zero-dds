...
wasm-bindgen XCDR1/XCDR2-Codec-Exposure fuer Browser. Kein Live-DDS — braucht WebSocket-Bridge.
...
Siehe `crates/ts-wasm/README.md` + `CHANGELOG.md` + `src/`.
...
- Codec-only; full DDS via crates/websocket-bridge als Server-Side-Gateway
...
- Kohärenz: (a) wired über C-FFI, (b) Konsumenten in Cross-Language-
  Bindings, (c) getestet mit lib only (kein cargo-test, da #[wasm_bindgen]-Crate)

## 8 Tests + Lints + Doc-Build

- `cargo test -p ts-wasm`: lib only (kein cargo-test, da #[wasm_bindgen]-Crate)
