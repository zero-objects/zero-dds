...
C#-P/Invoke-Bindings ueber `zerodds-c-api`. NativeAOT-kompatibel; DDS-PSM-Cxx-Idiom adaptiert auf .NET (IDisposable, record struct).
...
Siehe `crates/cs/README.md` + `CHANGELOG.md` + `src/`.
...
- C# Public Qos classes (`ZeroDDS.Qos.*`) + QosBridge\n- IDataWriterListener / IDataReaderListener Bridge
...
- Kohärenz: (a) wired über C-FFI, (b) Konsumenten in Cross-Language-
  Bindings, (c) getestet mit 1 cargo-test + 8 .NET-Smoke-Asserts

## 8 Tests + Lints + Doc-Build

- `cargo test -p cs`: 1 cargo-test + 8 .NET-Smoke-Asserts
