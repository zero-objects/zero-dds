...
Local DDS-Entity-API (Domain/Participant/Pub/Sub/Writer/Reader/Topic) per OMG-DDS-DCPS-1.4.
...
Siehe `crates/dcps/README.md` + `CHANGELOG.md` + `src/`.
...
- Kohärenz: (a) Public-API gewired, (b) Konsumenten in DCPS-Public-API/Bridges, (c) 583 cargo-tests grün

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-dcps`: 583 Tests
