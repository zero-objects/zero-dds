...
XRCE-Agent — die Server-Side-Bridge zwischen XRCE-Clients und einem DDS-Domain.
...
Siehe `crates/xrce-agent/README.md` + `CHANGELOG.md` + `src/`.
...
DDS-XRCE 1.0 §3 Agent-Operation.
...
- Kohärenz: (a) wired, (b) Konsumenten in Cross-Crate-Tests, (c)
  getestet mit 13 tests

## 8 Tests + Lints + Doc-Build

- `cargo test -p zerodds-xrce-agent`: 13 tests
- `cargo clippy -p zerodds-xrce-agent --tests -- -D warnings`: clean
